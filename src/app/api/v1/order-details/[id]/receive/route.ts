...
/** POST /api/v1/order-details/:id/receive — Nhận hàng (V21) */
export async function POST(req: NextRequest, ctx: Ctx) {
  try {
    ...
    const result = await DetailService.receiveDetail(
      decodeURIComponent(id),
      {
        actualReceived: Number(body.actualReceived ?? body.thucNhan),
        receivedDate: body.receivedDate || body.ngayNhan,
      },
      user,
      year
    );
