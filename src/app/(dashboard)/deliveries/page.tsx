...
export default function DeliveriesPage() {
  ...
  const filtered = useMemo(() => {
    return items.filter((d) => {
      // deliveries: pseudo status by actualQty
      const st =
        d.deleted ? "DELETED" : (d.actualQty || 0) > 0 ? "DONE" : "PENDING";
      if (!matchStatuses(st, statuses) && !statuses.includes("ALL")) {
        // also allow ALL
  ...
  return (
    <div className="space-y-4 max-w-full">
      ...
      <ListToolbar
        search={search}
        onSearch={setSearch}
        searchPlaceholder="Tìm mã GH, CT, khách hàng…"
        statuses={[
