  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-slate-500">Dữ liệu mock năm 2026</p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {cards.map((c) => (
            <Link
              key={c.title}
              href={c.href}
              className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm active:scale-[0.98] transition"
            >
              <div
                className={`w-9 h-9 rounded-xl ${c.color} mb-3 flex items-center justify-center text-white text-sm font-bold`}
              >
                {c.value}
              </div>
      ...
      <div className="mt-4 p-3.5 bg-sky-50 border border-sky-100 rounded-xl text-xs text-sky-800 leading-relaxed">
        <strong>Giai đoạn mock.</strong> Dữ liệu đang dùng ảo. Khi sẵn sàng sẽ
        chuyển sang Google Sheets thật qua Service Account – UI không đổi.
      </div>
    </div>
  );
}
